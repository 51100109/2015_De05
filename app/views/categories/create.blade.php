@extends('categories.main')

@section('title')
	Thêm danh mục phần mềm
@stop

@section('content')
	<div class="row">
	    <ol class="breadcrumb">
	        <li><a href="{{asset('admin/home')}}"></i> Trang Chủ</a></li>
	        <li><a href="{{asset('categories/index')}}"></i> Danh mục phần mềm</a></li>
	        <li class="active"> Thêm danh mục</li>
	    </ol>
		<h2><img src="{{asset('assets/image/icon.png')}}" alt="icon"> Danh Mục Phần Mềm</h2>
	</div>
	<div class="row"> 
	 	<div class="col-md-12">
			<ul class="nav nav-tabs">
				<li><a href="{{asset('categories/index')}}">Danh mục phần mềm</a></li>
			    <li class="active"><a href="{{asset('categories/create')}}">Thêm danh mục</a></li>
			    <li><a href="{{asset('categories/edit/0')}}">Chỉnh sửa danh mục</a></li>
			    <li><a href="{{asset('categories/delete/0')}}">Xóa nhà danh mục</a></li>
			    <li><a href="{{asset('categories/show')}}">Thông tin danh mục</a></li>
			</ul>
			<br>
			<div class="panel panel-primary">
		        <div class="panel-heading">
		            <h3 class="panel-title">Thêm Danh Mục Phần Mềm</h3>
		        </div>
				<div class="panel-body">
					<div class="alert alert-info alert-dismissable">
				        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				            <label>Điều khoản sử dụng</label>
				        <ol>
				            <li>Tên các danh mục không được giống nhau</li>
				            <li>Tên danh mục không được để trống</li>
				        </ol>     
		        	</div>
			        <div>
			        	<form class="well" href="{{asset('categories/create')}}" method="post"> 
							@if ($errors->any())
								<div class='alert alert-danger'>
									<ul>	
										{{ implode('', $errors->all('<span class="glyphicon glyphicon-exclamation-sign"></span> :message<br>')) }}
									</ul>
								</div>   
							@endif
						    <p>
						    	<label>Tên danh mục phần mềm</label>
						    	<input type='text' name='name' id='name' placeholder="Nhập tên danh mục" class='form-control' />
							</p>
							<p><button class="btn btn-default">Xác nhận</button></p>
						</form>
					</div>
		        </div>
		    </div>
		</div>
	</div>
@stop